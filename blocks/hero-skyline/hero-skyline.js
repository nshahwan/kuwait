...
export default function decorate(block) {
  ...
  if (textDiv) textDiv.classList.add('hero-skyline-text');

  // --- Auto-detect video from any <a> in the block ---
  const videoLink = findVideoLink(block);

  if (videoLink) {
    block.classList.add('hero-skyline-video');
    ...
    video.className = 'hero-skyline-video-bg';
  ...
  if (badgeText && textDiv) {
    ...
    badgeEl.className = 'hero-skyline-badge';
