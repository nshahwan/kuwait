...
const closeSearchOnFocusOut = (e, navTools) => {
  ...
  if (searchContainer && searchContainer.style.display !== 'none') {
    ...
    const contains = (el) => el && el.contains && el.contains(e.target);
    const isClickInside = contains(searchContainer)
      || contains(cancelContainer)
...
export default async function decorate(block) {
  ...
  const isAuthor = isAuthorEnvironment();
  // Default (published EDS) path. When previewing locally the content tree is
  // mounted under /content, so prefix the fragment path to match.
  const contentPrefix = window.location.pathname.startsWith('/content') ? '/content' : '';
  let navPath = `${contentPrefix}/${langCode}/nav`;
      ...
      uniqueLangs.forEach((raw) => {
        ...
        const resolveCountry = () => {
          if (regionPart) {
            ...
            return regionNames ? regionNames.of(region) : region;
          }
          return languageNames ? languageNames.of(langPart) : langPart.toUpperCase();
        };
