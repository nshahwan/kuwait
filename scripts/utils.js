 ...
 * - From ".../language-masters/en/path/to/content.html" returns "wknd-universal"
...
export function getLanguage() {
  if (!lang) {
    // Prefer an explicit supported-language token found anywhere in the path.
    // Covers preview/content paths where the language isn't at the expected
    // index (e.g. /content/ar) so RTL detection and fragment paths stay correct.
    const parts = window.location.pathname.split('/').filter(Boolean);
    const found = parts.find((part) => SUPPORTED_LANGUAGES.includes(part));
    if (found) {
      lang = found;
    } else {
 ...
 * @returns {boolean} true when the language is written right-to-left
 */
export function isRTL(langCode = getLanguage()) {
